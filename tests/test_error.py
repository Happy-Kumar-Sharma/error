...
# Add project root to sys.path so we can import error package without installing it
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

import error
from error.suggestions import SuggestionEngine
from error.formatting import Formatter
from error.analytics import _tracker
...
class TestDecorators(unittest.TestCase):
    def test_fallback(self):
        @error.fallback(default="fallback_value", exceptions=(ValueError,))
    ...
    def test_retry(self):
        calls = 0
        @error.retry(tries=3, delay=0.01, backoff=1.5, exceptions=(ValueError,))
        def retry_func():
            nonlocal calls
            calls += 1
            if calls < 3:
    ...
    def test_retry_with_jitter(self):
        calls = 0
        @error.retry(tries=3, delay=0.01, backoff=1.5, jitter=True, exceptions=(ValueError,))
...
class TestContextManagers(unittest.TestCase):
    def test_ignore(self):
        with error.ignore(ZeroDivisionError):
            _ = 1 / 0
            
        # Standard exceptions should not be ignored if specified otherwise
        with self.assertRaises(ZeroDivisionError):
            with error.ignore(ValueError):
                _ = 1 / 0

class TestFactory(unittest.TestCase):
    def test_custom_exception(self):
        MyException = error.create(
            "MyException",
...
class TestNewFeatures(unittest.TestCase):
    ...
    def test_privacy_scrub(self):
        ...
        scrubbed = error.Formatter.scrub_text(raw_msg)
        ...
        scrubbed_url = error.Formatter.scrub_text(url)
    ...
    def test_markdown_report(self):
        try:
            ...
            report = error.generate_markdown_report(exc)
    ...
    def test_testing_helpers(self):
        ...
        with self.assertRaises(AssertionError):
            error.assert_not_exposed(MockException())
        ...
        # Should pass
        error.assert_not_exposed(MockExceptionSafe())
